...
async def track_click(
    ...
    tid = str(tracking_id)
    # FastAPI automatically unquotes query parameters, no need for unquote(url)
    if not verify(tid, sig, url):
        # 🛡️ Sentinel: Fail securely to prevent Open Redirect vulnerability
        return HTMLResponse("<html><body><p>Ссылка недействительна или повреждена.</p></body></html>", status_code=400)
